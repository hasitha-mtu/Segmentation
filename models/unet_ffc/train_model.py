...
from model import unet_model, FFC
...
def train_model(epoch_count, batch_size, X_train, y_train, X_val, y_val, width, height,
    ...
    model_checkpoint_callback = ModelCheckpoint(
        f"{CKPT_DIR}/unet_ffc_model.h5",  # or "best_model.keras"
...
def make_or_restore_model(restore, width, height, input_channels):
    if restore:
        custom_objects = {'recall_m': recall_m,
                          ...
                          'FFC': FFC}
        saved_model_path = f"{CKPT_DIR}/unet_ffc_model.h5"
        ...
        return unet_model(width, height, input_channels)
