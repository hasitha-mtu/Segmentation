...
from keras.callbacks import (Callback,
                             CSVLogger)
import random
from model import unet_mobilenet_v2
...
def train_model(epoch_count, batch_size, X_train, y_train, X_val, y_val, width, height,
    ...
    model_checkpoint_callback = ModelCheckpoint(
        f"{CKPT_DIR}/unet_mobilenet_v2_model.h5",  # or "best_model.keras"
    ...
    cbs = [
        CSVLogger(LOG_DIR+'/unet_mobilenet_v2_logs.csv', separator=',', append=False),
...
def make_or_restore_model(restore, width, height, input_channels):
    if restore:
        ...
        saved_model_path = f"{CKPT_DIR}/unet_mobilenet_v2_model.h5"
